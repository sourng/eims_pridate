...
<tr class="{{ $row["total_all"] >= 5 ? "bg-gray text-white" : ""  }}">
    ...
    <td class="{{isset($no_name) ? "d-none" : ""}} text-center">
        {{mb_substr($row["node"]["node"]["gender"]["name"], 0, 1,'utf-8')}}</td>

    @for ($i = 1; $i <= DateHelper::daysOfMonth(request("year"),request("month")); $i++)
        @include(config("pages.parent").".includes.study.includes.attendance.includes.subbody.d") @endfor <td
        class="text-center" data-total-permission> {{$row["total_p"]}}</td>
